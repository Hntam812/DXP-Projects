...
def process_row_sale(row):
    ...
    sale_price_ecom = product_info[0][0]
    row["PRICE_ECOM"] = sale_price_ecom
    ...
    with open("data/sale_info.csv", mode="a", newline="") as file:
        fieldnames = [
            "UPC",
            "STORE_CODE",
            "PRICE",
        ...
        if os.stat("data/sale_info.csv").st_size == 0:
            writer.writeheader()
        ...
        writer.writerow(
            {
                ...
                "PRICE_ECOM": float(sale_price_ecom),
...
def process_batch_sale(df_batch):
    for _, row in df_batch.iterrows():
        process_row_sale(row)


def process_sale_product(yyyymmdd):
    print("CHECKING INFO OF PRODUCT WITH SALE PRICE")
    current_time = datetime.now()
    starttime = time.time()
    local_data_dir = config("LOCAL_DIR")
    blob_name = f"{yyyymmdd}/price_sale_of_product.csv"
    download_csv_file_from_blob(
        CONTAINER_CLIENT, blob_name, local_data_dir, "price_sale_of_product.csv"
    )
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        executor.map(process_batch_sale, chunks)
    print(f" Executed in {time.time() - starttime} seconds.")
    print(" Generate csv successfull ")

    upload_csv_to_blob(CONTAINER_CLIENT, yyyymmdd, "data/sale_info.csv")

    df = pd.read_csv("data/sale_info.csv")
    df["PRICE"] = df["PRICE"].apply(lambda x: f"{x:.2f}")
    df["PRICE_ECOM"] = df["PRICE_ECOM"].apply(lambda x: f"{x:.2f}")
    df.to_csv("data/sale_info.csv", index=False)

    output = f"data/price_sale_product_check_{yyyymmdd}.csv"
    error_rows = check_info_df_and_save(df, output)
    upload_csv_to_blob(CONTAINER_CLIENT, yyyymmdd, output)

    error_rows = pd.read_csv(f"data/price_sale_product_check_{yyyymmdd}.csv")
    ...
    if check == 0:
        ...
        link = config("LINK") + f"/{yyyymmdd}/price_sale_product_check_{yyyymmdd}.csv"
        yyyymmdd = current_time.strftime("%Y-%m-%d")
        send_notification(yyyymmdd, total_rows, link, success=False)

    remove_file(local_data_dir, "csv")
    print(f" Processing totally finished in {time.time() - starttime} seconds.")
